...
protocol MyReservationsPresenterDelegate: AnyObject {
    func presentReservations(reservations: [ReservationModel])
}

typealias PresenterDelegateMyReservations = MyReservationsPresenterDelegate & UIViewController

class MyReservationsPresenter {
    
    weak var delegate: PresenterDelegateMyReservations?
    
    public func setViewDelegate(delegate: PresenterDelegateMyReservations) {
    ...
    public func deleteReservation(reservationId: String) {
        var request = URLRequest(url: URL(string: .deleteReservationUrl + reservationId)!)
